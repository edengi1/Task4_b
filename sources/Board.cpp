...
Color Board::getCityColor(City c){return cities_colors.at(c);}
void Board::remove_stations(){researched.clear();}
...
Board::Board(){
    for (int c = (int)City::Algiers; c <= (int)City::Washington; c++){cubes[(City)c] = 0;}
    isCured.clear();
    researched.clear();}
void Board::remove_cures(){isCured.clear();}
void Board::add_cure(Color c){this->isCured.insert(c);}
void Board::reduce_disease(City c){
    if (this->cubes.at(c) > 0){
        if (this->isCured.count(cities_colors.at(c)) != 0){this->cubes.at(c) = 0;}
        else{this->cubes.at(c)--;}
        return;
    }
...
void Board::build_research_station(City c){researched.insert(c);}
int &Board::operator[](City c){return cubes[c];}
bool Board::is_clean(){
...
bool Board::is_connected(const City &c1, const City &c2){return connections.at(c1).count(c2) > 0;}
bool Board::is_research_station(City c){return (researched.count(c) != 0);}
bool Board::check_cure(Color c){return (isCured.count(c) != 0);}
