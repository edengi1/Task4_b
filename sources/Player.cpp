...
Player &Player::build(){
    if (cards.count(city) == 0){throw std::invalid_argument{" the player dont have a  " + CitytoString(city) + " card!"};}
...
Player &Player::drive(const City c){
    if (this->city == c){throw std::invalid_argument{" you already in this city " + CitytoString(this->city)};}
    if (Board::is_connected(this->city, c)){
        this->city = c;
        if (this->type == "Medic" && this->board.check_cure(Board::getCityColor(c))){this->board[c] = 0;}
        return *this;
    }
...
Player &Player::fly_direct(const City c){
    ...
    throw std::invalid_argument{" you do not have the card of: " + CitytoString(c)};
}
...
Player &Player::fly_charter(const City c){
    ...
    throw std::invalid_argument{" you do not have the card of " + CitytoString(this->city)};
}
...
Player &Player::discover_cure(Color c){
    ...
    throw std::invalid_argument{" Not heve research station "};
}
